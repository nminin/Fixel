...
fun <V: TextView> V.setCursorDrawableColor( @ColorInt color: Int) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
        val gradientDrawable = GradientDrawable(GradientDrawable.Orientation.BOTTOM_TOP, intArrayOf(color, color))
        gradientDrawable.setSize(2.spToPx(this.context).toInt(), this.textSize.toInt())
    ...
    try {
        ...
        val tintedCursorDrawable = TextView::class.java.getDeclaredField("mCursorDrawableRes")
            ...
            .getInt(this)
            .let { ContextCompat.getDrawable(this.context, it) ?: return }
